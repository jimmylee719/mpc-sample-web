...
| ID | 標題 | 核心 |
|---|---|---|
| 1-1 | **先把機器裡的東西玩過一遍** | 官方 Tutorial 前三步：播放示範專案、換音色、排序列（2026-09-20 新增） |
| 1-2 | 你的第一個 30 分鐘 | 開機→錄音→切片→序列→存檔（原型已完成） |
| 1-3 | 讀懂面板：紅字的世界 | SHIFT 邏輯、LED 語言、旋鈕絕對位置與 Takeover |
| 1-4 | 開機第一件事：韌體更新 | ⚠️ 唯一需要電腦的課；Windows 10 例外 |
| 1-5 | 專案、樣本、記憶卡 | 背景自動存檔 ≠ 備份 |

**Season 2 · 取樣工藝**

| ID | 標題 | 核心 |
|---|---|---|
| 2-1 | **把外面的聲音抓進來** | 手機／電腦 USB、唱盤經唱頭放大器、混音器；四個錄音設定與三種停止方式 |
...
| ~~不得轉錄任何影片為文字，也不得翻譯字幕~~ **（2026-08-10 放寬）** | 原規則連「用自己的話摘要」都禁掉，超出法律要求。**現在可以寫自己的話的中文摘要**（`summary`，≤ 200 字，且只有看過的人能寫）。仍然禁止**重製與改作**：逐字稿、完整翻譯的字幕檔、跟原片等長的文字版 |
| 不得下載影片或抽取其音訊當素材 | 素材一律自錄或 CC0 |
| 不得提供他人音樂素材下載 | 素材庫一律自錄或 CC0，保留原始檔佐證 |
| 不得教導破解或第三方韌體 | 僅教官方韌體 |
| ~~不得將串流平台指名為取樣來源~~ **（2026-08-09 解除）** | 技術教學可以指名手機、電腦上的任何播放來源，包含串流平台。**但每一處指名的地方都必須附上版權提醒並連結至 2-2**。2-2 仍須完整保留「平台服務條款本身即禁止錄製」這項事實 |

### 12.2.1 影片的正當用途（2026-08-10 修訂）

影片開放作為四種用途：**發現問題**、**交叉比對多方說法**、**延伸觀看**（手感、律動、音色這類文字教不了的東西），以及 **2026-08-10 新增的中文摘要**。

| 可以 | 不可以 |
|---|---|
| 在課程頁與曲風頁嵌入或連結影片 | **逐字稿、完整翻譯的字幕檔、跟原片等長的文字版** |
| **看過之後用自己的話寫中文摘要**（≤ 200 字） | 把摘要寫成影片的替代品，讓人不必看原片 |
| 引用單句以內的短句並標明頻道 | 大段引用原話 |
| 用嵌入參數開啟 YouTube 自己的字幕與自動翻譯 | 把 YouTube 的字幕檔抓下來存進本站 |
| 多支影片交叉比對，找出說法分歧 | 把影片說法直接當事實寫進課程 |
| 記錄頻道、標題、網址、選它的理由 | 宣稱影片內容正確而未經官方或實測佐證 |

**摘要的執行限制**：`summary` 只有實際看過影片的人能寫，驗證腳本強制
「有 `summary` 就必須 `reviewed: true`」。Claude 沒有影像與聲音輸入、看不了影片，
因此不會產生任何 `summary`；它負責的是欄位、驗證規則與前台呈現。
...
## 14. 待實測清單（未驗證前不得寫成肯定語氣）

以下十一項只能靠實機或官方回覆才能確定。前台 `/reference/firmware` 會原樣列出，
不會寫成肯定語氣。已經查清楚的項目移到下一小節，過程留著備查。
...
| V-14 | 單一專案檔案大小與 8 GB 可容納專案數 | 1-5 |
...
| V-19 | 內建儲存是否為 eMMC | 1-5 |
| V-20 | USB MIDI 同步時 Flex Beat 是否產生爆音 | 疑難排解 |
| V-21 | 內建 3W 喇叭是否為單聲道 | 1-3 |
| V-7 | 與 MPC3 桌面軟體互通：手冊稱不支援 vs FAQ 稱可開啟 | 3-9 備援路線 |
| V-22 | 16 Levels Tune 型式的音程間隔 | 3-4 |
| V-24 | 韌體更新是否影響既有專案（官方三次更新說明都沒寫） | 1-4 |
| V-25 | auto-snapping 是哪一個功能（只出現在功能列表，內文無說明） | 2-6 |

### 14.1 已查清（結論與依據見 `/reference/firmware` 的「已排除的疑問」）

| 編號 | 項目 | 結論 |
|---|---|---|
| V-8 | Recall 擷取長度 | 音訊 25 秒；序列 Recall 撈的是上一個循環，不是秒數 |
| **V-12** | **匯出檔案存在哪裡；未插 microSD 能否取出作品** | **掛載到電腦的是 microSD，不是內建 8 GB。沒有卡就傳不出來** |
| V-16 | 盒裝是否附 MIDI 轉接線 | 沒附。FAQ 那句講的是規格型別，不是盒裝內容 |
| V-23 | 電池續航 | 手冊約 5 小時；FAQ 的 6 小時視為行銷用語 |
| V-26 | Splice 怎麼搭配這台機器 | 沒有機上整合（本機無 Wi-Fi），要在電腦或手機下載再傳進卡 |
| V-27 | lazy-chopping 是哪一個功能 | Chop Type 的 Manual 模式 |
| V-28 | fixed-length sampling 是哪一個功能 | Input Config 的 Rec Length 設成 SEQ |
