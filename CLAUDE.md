...
- ❌ 轉貼論壇貼文原文
- ⚠️ **（2026-08-10 修訂）影片可以用自己的話寫摘要，但不可逐字轉錄或整段翻譯**
  - 原本的規則把「用自己的話講這支影片在幹嘛」也一起禁掉了，那超出法律要求，已放寬
  - 仍然禁止的是**重製與改作**：逐字稿、完整翻譯的字幕檔、跟原片等長的文字版
  - 摘要必須明顯短於原片、是自己的話、標明頻道與出處
  - **只有實際看過影片的人能寫摘要**（`reviewed: true`）。沒看過就寫等於編造
- ❌ 提供他人音樂素材下載 → 素材庫一律自錄或 CC0
- ❌ 下載影片、抽取影片音訊當素材
- ❌ 教導破解或第三方韌體
- ✅ **（2026-08-09 解除）技術教學可以指名串流平台為取樣來源**。條件：每一處都要附版權提醒並連到 2-2；2-2 必須保留「平台服務條款本身即禁止錄製」這項事實

### ✅ YouTube 的正當用途（2026-08-10 修訂）

| 可以 | 不可以 |
|---|---|
| 用官方 embed 或連結，把影片放在課程頁與曲風頁 | **逐字稿、完整翻譯的字幕、跟原片等長的文字版** |
| **看過之後用自己的話寫一段中文摘要**（`summary` 欄位，≤ 200 字） | 把摘要寫成「等於看過影片」的替代品 |
...
**摘要誰能寫**：只有實際看過那支影片的人。Claude 沒有影像與聲音輸入，**看不了影片**，
所以 Claude 永遠不會、也不能產生 `summary`。它只能建好欄位、驗證規則與前台呈現，
內容由看過的人填。驗證腳本會強制 `summary` 存在時 `reviewed` 必須為 `true`。
...
`docs/PROJECT-PLAN.md` §14 列有待實測清單，§14.1 是已經查清楚的。**未驗證前一律標示為「尚未驗證」，不可寫成肯定語氣。**

（2026-09-20 更新）V-12 已查清：掛載到電腦的是 microSD，不是內建 8 GB，沒有卡就傳不出來。`3-9` 與配件清單依此撰寫。剩下的項目多半要實機才能確認，查不到就照實寫查不到，不要用推測補洞。
